...
Intuitively, for a query tuple $t$, view mapping $M$ is valid iff there exists a view tuple $t'$ in $V$ such that $t'$ is {\em visible} in $t$ under $M$. The reasoning depends on examining the lambda variables, the head variables and the predicates of views.  % by \rba\ in \cite{wu2018data}. 
